...
%   options.maxmem      max number of vectors to keep in memory (needed for first full orthogonal Lanczos iterations to estimate extreme eigenvalues) 
%   options.variant     'backslash', 'pcg' or 'multishiftcg'
%   options.tol_solver  tolerance for inner solver
%   options.max_iter    maximum number of iterations for inner solver
...
if isfield(options, 'tol_solver') == 0
        options.tol_solver = options.tol*1e-2;
end

if isfield(options, 'variant') == 0
...
if isfield(options, 'xi') == 0 && (isfield(options, 'eigmin') == 0 || isfield(options, 'eigmax') == 0)
...
[U, X, iter, k] = lyap_ratkrylov_util(A,c, normc, options);
