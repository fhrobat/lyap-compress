function [U, Y, iter, k, m, errest] = lyap_compress_util(mult,c,normc,options, Q, v1, v2, alpha, beta) 
...
Y = lyap(ProjMat, -normc^2*( (Utilde' * w) * (Utilde' * w)')); 
Y = (Y + Y')/2;

% estimate residual norm
err = sqrt(2)*beta(end) * norm( (Wtilde(end,:) * Utilde) * Y, 'fro')/normc^2;
...
if true_res
        ...
        R3 = R(:, end);
        Rperm = [R2, R1, -R3];
        error = norm(R * Rperm', 'fro') / norm(c)^2;
        errest = [errest,[iter,err, error]];
else
...
while iter + m < n
    ...
    % Wtilde orthonormcl basis of Q(S, [w;0 , 0;e_m], xi)
